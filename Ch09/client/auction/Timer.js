...
// Pg 427, initialize the timeLeft variable in the state, using the end time value sent in the props from the Auction component
export default function Timer (props) {
  const classes = useStyles()
    const [timeLeft, setTimeLeft] = useState(calculateTimeLeft(new Date(props.endTime)))

    // Pg 427, to implememnt the time countdown functionality, we will use setTimeout in a useEffect hook in the timer component
    useEffect(() => {
      let timer = null
      if(!timeLeft.timeEnd){
        timer = setTimeout(() => {
    ...
    // Pg 428, render the view to show clearout timer
